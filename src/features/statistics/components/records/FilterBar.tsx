...
import type { FilterKey, RecordFilters } from './types'
  ...
  return (
    <div className="relative flex flex-wrap items-center gap-2">
      {activeDefs.map((def) => (
        <button
          key={def.key}
          onClick={() => setOpen((prev) => (prev === def.key ? null : def.key))}
        ...
        >
          ...
          <span
            ...
            onClick={(e) => {
              e.stopPropagation()
              removeFilter(def.key, def.reset)
            }}
      ...
      {currentDef !== null && (
        <div className="absolute left-0 top-full mt-2 z-50 min-w-[200px] rounded-xl border border-gray-200 bg-white shadow-xl py-1.5">
          ...
          {currentDef.options(templates).map((opt) => {
            const isActive = (filters[currentDef.key] as string) === opt.value
            return (
              <button
                key={opt.value}
                onClick={(e) => {
                  e.stopPropagation()
                  applyFilter(currentDef.key, opt.value)
                }}
